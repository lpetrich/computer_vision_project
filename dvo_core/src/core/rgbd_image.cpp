...
RgbdImagePtr RgbdCamera::create(const cv::Mat& intensity, const cv::Mat& depth) const
{
	...
	result->intensity = intensity.clone();
	result->depth = depth.clone();
	hasSameSize(depth);
	result->initialize();
...
void RgbdCameraPyramid::build(size_t levels)
{
	...
	for(size_t idx = start; idx < levels; ++idx)
	{
		...
		// std::cout << "\tCamera pyramid level: " << idx << "/" << levels << " initialized.\n";
	}
}
...
{
 	TRACE()
 	// std::cout <<  "\t" << !depth.empty() << "\n";
	return !depth.empty();
}

bool RgbdImage::hasSameImageSize(const cv::Mat& img) const
{
	TRACE()
	bool check = (img.cols == width && img.rows == height);
  	// std::cout << "\t" << check <<"\n";
	return img.cols == width && img.rows == height;
}
