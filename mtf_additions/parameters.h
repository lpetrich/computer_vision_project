...
#define KINECT_DEV_NAME "kinect_cam"
	...
	namespace params{
		...
		inline bool postProcessParams(){
			...
			if((img_source == SRC_IMG) || (img_source == SRC_DISK) || (img_source == SRC_VID)){
				...
				actor = "Live";
				if (img_source == SRC_USB_CAM) {
					seq_name = USB_DEV_NAME;
				} else if (img_source == SRC_KINECT_CAM) {
					seq_name = KINECT_DEV_NAME;
				} else {
					seq_name = FW_DEV_NAME;
				}
